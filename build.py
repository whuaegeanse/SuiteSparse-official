...
def parse_args():
    ...
    parser.add_argument("--build_static", dest="build_static", action="store_true")
    ...
    parser.set_defaults(build_static=False)
    ...
    if args.build_shared:
        args.cmake_config_args.append("-DBUILD_SHARED_LIBS=ON")
    else:
        args.cmake_config_args.append("-DBUILD_SHARED_LIBS=OFF")
    
    if args.build_static:
        args.cmake_config_args.append("-DBUILD_STATIC_LIBS=ON")
    else:
        args.cmake_config_args.append("-DBUILD_STATIC_LIBS=OFF")
...
def main():
    ...
    BUILD_LIBS = "SuiteSparse_config:Mongoose:AMD:BTF:CAMD:CCOLAMD:COLAMD:CHOLMOD:CXSparse:LDL:KLU:UMFPACK:RBio:SPQR:GraphBLAS:SPEX"
